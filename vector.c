...
//Memory O(1)
//Runtime: O(1) freeing runtime unkown, probably O1
...
//Memory O(N) where N is the array size. Note, that if N is really small, the pointer sizes and longs will take the majority of space, 
...
//Memory max O(N), probably O(1)
//Runtime: O(N) where N is the element num that has to be shifted + inserted.
void insert_to_index_and_shift_vector(vector* v, int x, int index)
{
  size_t cur_num_elems = v->num_elems;
...
//Memory O(N) where N is the array size. Note, that if N is really small, the pointer sizes and longs will take the majority of space, 
//while when N gets big, they get less significant.
//Runtime: O(N) where N is the element num (because of possible resize)
void push_back(vector* v, int x)
{
  insert_at(v, v->num_elems, x);
}

//Memory O(N) where N is the array size. Note, that if N is really small, the pointer sizes and longs will take the majority of space, 
//while when N gets big, they get less significant.
//Runtime: O(N) where N is the element num.
...
//Memory O(N) where N is the array size. Note, that if N is really small, the pointer sizes and longs will take the majority of space, 
//while when N gets big, they get less significant.
//Runtime: O(N) where N is the element num due to possible resize.
...
//Memory O(N) or O(1) depending on how memmove works
//Runtime: O(N) where N is the element num that has to be shifted + deleted.
...
//Memory O(1)
//Runtime: O(1) malloc is the slowest.
vector_iterator begin(const vector* v)
{
  vector_iterator iterator = {v, 0};
  return iterator;
}
...
//Memory O(1)
//Runtime: O(N) where N is the sum of the element nums
